...
func _add_card(unit, is_active: bool) -> void:
	...
	portrait.rect_min_size = Vector2(PORTRAIT_SIZE, PORTRAIT_SIZE)
	var portrait_style := StyleBoxFlat.new()
	portrait_style.bg_color = team_color if is_active else team_color.darkened(0.35)
	portrait_style.set_corner_radius_all(2)
	portrait.add_stylebox_override("panel", portrait_style)
	hbox.add_child(portrait)

	var initial := Label.new()
	initial.text = unit.unit_name.left(1).to_upper()
	initial.align = Label.ALIGN_CENTER
	initial.valign = Label.VALIGN_CENTER
	initial.add_color_override("font_color", Color(1, 1, 1, 0.9))
	initial.set_anchors_and_margins_preset(Control.PRESET_WIDE)
